...
completedLearningHistory: Record<number, string>; // 单元编号 -> 学习日期(ISO string)
createdAt: string;
}
...
const [completedLearningHistory, setCompletedLearningHistory] = useState<Record<number, string>>({}); // 单元编号 -> 学习日期
...
// Auto-detect based on browser language when no preference is saved
...
setCompletedLearningHistory(plan.completedLearningHistory || {});
...
// Save completed ranges and learning history whenever they change
useEffect(() => {
if (studyPlan && currentPlanId) {
const updatedPlans = savedPlans.map(p =>
p.id === currentPlanId
? { ...p, completedRanges: Array.from(completedRanges), completedLearningHistory }
: p
);
localStorage.setItem(PLANS_LIST_KEY, JSON.stringify(updatedPlans));
}
}, [completedRanges, completedLearningHistory, currentPlanId]);
...
const toggleRangeCompletion = (rangeId: string, unitStart?: number, unitEnd?: number, taskDate?: Date) => {
// 切换完成状态
const wasCompleted = completedRanges.has(rangeId);
...
// 如果是标记为完成（非取消），同时更新学习历史
if (!wasCompleted && unitStart !== undefined && unitEnd !== undefined && taskDate) {
setCompletedLearningHistory(prev => {
const newHistory = { ...prev };
for (let i = unitStart; i <= unitEnd; i++) {
// 只有当之前没有学习记录时才添加
if (!newHistory[i]) {
newHistory[i] = taskDate.toISOString();
}
}
return newHistory;
});
...
// 1. 收集所有被标记为完成的单元，以及所有已有学习历史的单元
const allCompletedUnits = new Set<number>();
const newLearningHistory: Record<number, string> = { ...completedLearningHistory };

// 首先，将所有已有学习历史的单元加入已完成列表
// 这样即使在当前计划中没有被标记为完成，只要有学习历史，就认为是已学习
for (const unitStr of Object.keys(newLearningHistory)) {
const unit = parseInt(unitStr);
if (unit >= 1 && unit <= studyPlan.totalAmount) {
allCompletedUnits.add(unit);
}
}

// 然后，从当前计划中收集新标记为完成的单元
studyPlan.tasks.forEach(task => {
// 从newLearning中收集
task.newLearning.forEach(range => {
if (completedRanges.has(range.id)) {
for (let i = range.start; i <= range.end; i++) {
allCompletedUnits.add(i);
// 记录学习日期（如果是首次学习）
if (!newLearningHistory[i]) {
newLearningHistory[i] = task.date.toISOString();
}
}
}
});
...
// 如果复习被完成，也要确保有学习日期记录
if (!newLearningHistory[i]) {
// 尝试从该天的newLearning中找学习日期
task.newLearning.forEach(nl => {
if (nl.start <= i && nl.end >= i && !newLearningHistory[i]) {
newLearningHistory[i] = task.date.toISOString();
}
});
}
...
// 2. 区分已完成学习（有学习历史）和未完成的单元
const learnedUnits: number[] = [];
const neverLearnedUnits: number[] = [];

for (const unit of allCompletedUnits) {
if (newLearningHistory[unit]) {
learnedUnits.push(unit); // 有学习记录，进入复习
} else {
neverLearnedUnits.push(unit); // 无学习记录，进入新学习
}
}

// 3. 收集剩余未完成的单元
const allUnits = Array.from({ length: studyPlan.totalAmount }, (_, i) => i + 1);
for (const unit of allUnits) {
if (!allCompletedUnits.has(unit)) {
neverLearnedUnits.push(unit); // 未完成的进入新学习
}
}

// 排序
learnedUnits.sort((a, b) => a - b);
neverLearnedUnits.sort((a, b) => a - b);

// 4. 构建已完成单元的原始学习日期映射
const completedLearningDates = new Map<number, Date>();
for (const unit of learnedUnits) {
if (newLearningHistory[unit]) {
completedLearningDates.set(unit, new Date(newLearningHistory[unit]));
}
}
...
shuffle: shuffle,
isReplan: true,
completedUnits: learnedUnits, // 只传入真正学习过的单元
...
setCompletedRanges(new Set());
setCompletedLearningHistory(newLearningHistory);
...
plan: newPlan,
completedRanges: [],
completedLearningHistory: newLearningHistory,
...
const uncompletedCount = neverLearnedUnits.length;
...
onCheckedChange={() => toggleRangeCompletion(range.id, range.start, range.end, task.date)}
className="mt-0.5"
/>
